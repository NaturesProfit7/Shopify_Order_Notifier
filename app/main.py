...
async def shopify_webhook(request: Request):
    ...
    if not marked:
        # Race condition - другой процесс уже обработал
        log_event("webhook_race_condition", order_id=str(order_id))
        return {"status": "duplicate", "order_id": str(order_id)}

    # Сохраняем chat_id из переменной окружения
    import os
    ...
    try:
        ...
        for attempt in range(1, 4):
            try:
                result = send_file(pdf_bytes, pdf_filename, caption=pdf_caption)
        ...
        log_event("pdf_generation_failed", order_id=str(order_id), error=str(e))
        raise
    ...
    for attempt in range(1, 4):
        try:
            result = send_file(vcf_bytes, vcf_filename, caption=caption)
    ...
    draft = render_simple_confirm(order_full)

    for attempt in range(1, 4):
        try:
            # Отправляем как обычный текст (позже добавим кнопки)
            result = send_text(draft)

            # Сохраняем message_id для будущих кнопок
